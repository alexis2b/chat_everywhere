...
/* Some modifications by A. de Bernis <alexis@bernis.org>
 *  - added function md5_hash_string
 *  - added string.h for function strlen
 */
...
/* MD5_hash_string
 * hash a simple string and returns the hexa string hash
 * Added by A. de Bernis <alexis@bernis.org>
 */
void
MD5_hash_string(const char *to_hash, char hash[33])
{
	struct MD5_ctx ctx;
	unsigned int i;
	...
	MD5_start(&ctx);
	for(i = 0; i < strlen(to_hash); i++) {
		MD5_feed(&ctx, to_hash[i]);
	}
	MD5_stop(&ctx, digest);

	for(i = 0; i < 16; i++) {
		snprintf(duplet, 3, "%02x", digest[i]);
		hash[2*i] = duplet[0];
		hash[2*i + 1] = duplet[1];
	}
	hash[32] = '\0';
